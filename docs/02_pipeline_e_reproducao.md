...
`dados/` contém cursos, IES, arquivos de aluno e tabelas auxiliares usadas pela pipeline. `referencia/` contém dicionários, leia-me, notas informativas e filtros do INEP. Cada pasta `ANO/` contém todos os arquivos CSV brutos (cursos, IES, alunos) e os materiais de referência (dicionários, notas) para um ano específico.
...
Uma execução pronta para uso deve terminar com `Total de Erros Críticos: 0`. Qualquer detalhe fica em `data/processed/validacao/ocorrencias_validacao.csv`.